...
def extract_filenames(file_path):
  ...
  if frontmatter_match:
    ...
    try:
      ...
      # Extract og_image and title from frontmatter
      og_image = frontmatter.get("og_image")
      title = frontmatter.get("title", "")
      if og_image:
        # Use title as caption, fallback to filename if title is empty
        caption = title.strip() if title.strip() else os.path.basename(og_image).rsplit(".", 1)[0].replace("_", " ")
        caption_filename_pairs.append((caption, os.path.basename(og_image)))
    except yaml.YAMLError:
      # Skip invalid YAML frontmatter
      pass

  # Extract image references (e.g., ![alt text](image.jpg))
  img_matches = re.findall(r"!\[(.*?)\]\(([^)]+\.(?:jpg|jpeg|png))\)", content)
  for caption, filename in img_matches:
    # Use filename as fallback if caption is empty
    caption = caption.strip() if caption.strip() else os.path.basename(filename).rsplit(".", 1)[0].replace("_", " ")
